...
- Set `HOMEBREW_NO_UPGRADE_AUTO_UPDATES_CASKS=1` in the managed shell init so `brew upgrade` no longer upgrades casks with `auto_updates true` (Chrome, Slack, Docker Desktop, …), restoring the behaviour a recent brew change removed (https://github.com/Homebrew/brew/pull/21882); the sparkdock TUI's brew upgrade sets the same variable. Opt back in per run with `brew upgrade --greedy`
