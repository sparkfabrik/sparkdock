...
- Added `sjust macos-defaults-init-overrides` recipe that drops a commented overrides template at `~/.local/spark/macos-defaults/overrides.yml` so users have a real file to edit (with examples for the most commonly customised settings) instead of starting from an empty path
- Added `sjust macos-defaults-info` recipe that displays the curated macOS-defaults profile grouped by category, with per-setting descriptions, current-state markers (`✓` aligned, `✗` drifted, `+` unset), and override examples; renders through `gum format` + `gum pager` when interactive, falls back to plain Markdown for piping
- Added per-key macOS-defaults snapshot/undo: each apply records `{was_set, type, value}` only for the keys sparkdock managed; `sjust macos-defaults-undo` replays those exact keys (writing back originals or `defaults delete`-ing what we created) and leaves any unrelated changes the user made in the same preference domain intact
- Added bash 4+ runtime check at the top of `sjust/scripts/macos-defaults/lib.sh` so the macos-defaults scripts fail with a clear "install bash via brew" message rather than a cryptic `declare: -A: invalid option` if Apple's bash 3.2 is on PATH
- Added a CI assertion in `test-ansible-playbook.yml` that runs `defaults read` on the curated keys after the Ansible task, catching cases where a write succeeds but lands on the wrong domain (e.g. wrong bundle-ID case)
- Added `NO_COLOR` env-var honouring in `sjust macos-defaults` apply / dry-run output, plus automatic colour-off when stdout is not a TTY
- Added `flock`-based concurrency lock so two simultaneous `sjust macos-defaults` runs on the same machine cannot interleave writes
...
- Trimmed the curated macOS-defaults set down to 8 minimal entries — only filesystem hygiene (`.DS_Store` on networks/USB), Time Machine prompt suppression, secure keyboard entry in Terminal, expanded save / print panels, and UTF-8 in TextEdit. Everything else (dock, Finder layout, trackpad, accessibility, keyboard repeat, Activity Monitor, screencapture, Safari) was removed because it is either personal preference or sandboxed and silently no-op on macOS 14+; users who want these keys set them via `sjust macos-defaults-init-overrides`
- Corrected Terminal bundle-ID case in `config/macos/defaults.yml` (`com.apple.terminal` → `com.apple.Terminal`); the lowercase variant does not exist on macOS so the setting was previously a silent no-op despite passing idempotency checks
- Switched undo from whole-domain `defaults import` to per-key replay so it only touches the keys sparkdock wrote, leaving any unrelated changes the user made in the same preference domain (e.g. dock magnification, custom Safari preferences) intact
- `bin/common/logging.sh` log helpers (`log_info`/`log_success`/`log_warn`/`log_error`/`log_section`) now write to stderr in the no-gum fallback path, matching gum's own default; this stops `var="$(my_func)"` capture from being contaminated by log lines when gum is missing
- `sjust macos-defaults-info` now reads the live value of each setting and tags it `✓` (aligned), `✗` (drifted), or `+` (unset) so users see exactly what apply would change before running it
- Refactored `sjust macos-defaults*` recipes to thin wrappers; complex logic moved into standalone shell scripts under `sjust/scripts/macos-defaults/` (`lib.sh`, `apply.sh`, `undo.sh`, `docs.sh`, `info.sh`, `init-overrides.sh`) to match the project's pattern for `openspec/configure.sh` and `rtk/setup.sh`
