...
- Added globally managed optional AI harness skill categories with `sf-harness-category list|enable|disable`, persistent user selection, conflict-safe removal, category health in `sf-harness-status`, and shared `sjust`/Linux `ajust` support; system skills remain mandatory and existing manifests migrate without reinstalling optional categories
