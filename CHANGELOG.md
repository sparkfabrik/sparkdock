...
- Added remote URL self-healing to `sparkdock-agents-sync`: the cached upstream clone's `origin` is repointed to the configured repository URL before every fetch, so upstream repository renames are absorbed without re-cloning
