...
- Added a Claude Code coding-agent service-account gate: a sparkdock-managed `PreToolUse` hook (`sjust/scripts/claude-sa-gate.py`) that watches GCP commands (`gcloud`, `terraform`, `tofu`, `terragrunt`, `bq`, `gsutil`) and checks whether the agent is running as its coding-agent service account, requiring both `GOOGLE_APPLICATION_CREDENTIALS` and the gcloud-honored `CLOUDSDK_AUTH_CREDENTIAL_FILE_OVERRIDE` to point at a key in the agent SA project (as set by `fs-cli-agent-exec`), since gcloud ignores the former for its own authentication. The gate is inert until the organization both activates a phase with `SF_AGENT_SA_GUARD` (`warn` surfaces a remediation message and allows the command; `enforce` blocks it) and names the project with `FSCLI_AGENT_SA_PROJECT`; no project name is hard-coded, and `claude-sa-gate-info` reports which condition is missing. Quoted spans are ignored when matching, so a CLI named inside a commit message or a grep pattern is not mistaken for an invocation, and credential-free subcommands (`terraform fmt`, `terraform validate`, `gcloud --version`, `gcloud config list`) are never blocked. In both phases it blocks `gcloud auth login` and `gcloud auth application-default login` in command position, since an agent must never mint human credentials. The docstring records what the gate cannot see (a command reached through `bash script.sh`, `sh -c`, a Makefile or a `just` recipe) so it is treated as a guardrail against drift rather than a control against an adversarial agent. Registered during provisioning like the gh gate (managed with `sjust claude-sa-gate-{enable,disable,info}`), reuses the shared `claude_settings.py` plumbing (idempotent, atomic write with timestamped backup, preserves other hooks), and can be bypassed by setting `SPARKDOCK_SA_GATE=0` in the environment before launching Claude Code.
