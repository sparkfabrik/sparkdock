...
- Added globally managed optional AI harness skill categories with styled, visually separated `sf-harness-category [list]|enable|disable` output, concise failure messages, persistent user selection, conflict-safe removal, category health and unmanaged skill paths in `sf-harness-status`, and shared `sjust`/Linux `ajust` support; system skills remain mandatory and existing manifests migrate without reinstalling optional categories
