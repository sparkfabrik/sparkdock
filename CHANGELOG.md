...
- Added a self-update step to the `sparkdock tui` `Update everything` action: it now force-syncs the `/opt/sparkdock` install to upstream `master` (stashing any local changes) before provisioning, matching bare `sparkdock`, so the Sparkdock status clears; the dashboard advises a relaunch when the update rebuilds the hub's own binary
