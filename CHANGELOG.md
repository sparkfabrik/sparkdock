...
- Added a Claude Code coding-agent service-account gate: a sparkdock-managed `PreToolUse` hook (`sjust/scripts/claude-sa-gate.py`) that watches GCP commands (`gcloud`, `terraform`, `bq`, `gsutil`) and checks whether the agent is running as its coding-agent service account (via `GOOGLE_APPLICATION_CREDENTIALS` pointing at a key in the agent SA project, as set up by `fs-cli-agent-exec`). The gate is inert until the organization both activates a phase with `SF_AGENT_SA_GUARD` (`warn` prints a remediation message and allows; `enforce` blocks with exit 2) and names the project with `FSCLI_AGENT_SA_PROJECT`; no project name is hard-coded. In both phases it always blocks `gcloud auth login` and `gcloud auth application-default login`, since an agent must never mint human credentials. Registered during provisioning like the gh gate (managed with `sjust claude-sa-gate-{enable,disable,info}`), reuses the shared `claude_settings.py` plumbing (idempotent, atomic write with timestamped backup, preserves other hooks), and can be bypassed at runtime with `SPARKDOCK_SA_GATE=0`.
