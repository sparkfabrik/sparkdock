...
- Added `sjust macos-doctor`, a pluggable macOS health-check command with ten checks: CPU load, memory pressure, disk space, orphaned launchd jobs, local APFS snapshots, developer caches, Homebrew health, Spotlight index scope, reclaimable Docker disk and shell startup time. The report is read-only. `macos-doctor-fix <check>` acts on one check at a time behind a confirmation that lists the exact items it will remove, previewable with `dry-run`, reversible through `macos-doctor-undo` where the fix moves files rather than deleting them. Adding a check means dropping a file into `sjust/scripts/macos-doctor/checks/`. Wired into `sparkdock tui` as a status row and a `Run diagnostics` action on `D`
