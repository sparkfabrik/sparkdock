...
- Added `sjust claude-usage-install [version]` and `sjust claude-usage-uninstall` recipes that wrap the official cross-platform claude-usage installer (https://github.com/sparkfabrik/claude-usage) — auto-detects macOS/Linux, arch, and desktop reader; idempotent; optional version pin
