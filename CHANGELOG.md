...
- Added `sjust claude-fix-settings` and `sjust claude-fix-settings-info` recipes that repair sparkdock-managed Claude Code hooks in `~/.claude/settings.json` — normalize the caveman hook node path to a stable Homebrew symlink and remove the claude-usage session hooks; idempotent, atomic write with timestamped backup, no-op on a missing/corrupt settings file. Also run automatically at the end of the caveman setup so it self-heals every provisioning pass on macOS and Linux
