...
- Fixed provisioning failing with `sudo: a terminal is required to read the password` when installing casks whose installer shells out to its own internal `sudo` (e.g. `docker-desktop`'s `/usr/local/bin` symlink setup, `zoom`'s pkg installer): those internal calls don't honor the `homebrew_cask` module's `sudo_password`/`SUDO_ASKPASS`, so the sudo credential cache is now primed via stdin right after the become password is collected, with a bounded 60-minute background refresh keeping it warm through the cask/package install phase
