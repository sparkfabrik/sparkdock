...
- Fixed `skhd` install failing on fresh machines because its upstream GitHub owner renamed from `koekeishiya` to `asmvik`, leaving `brew tap koekeishiya/formulae` pointing at a redirecting/dead path: repointed the tap to `asmvik/formulae` and the package to `asmvik/formulae/skhd`, and added `koekeishiya/formulae` to `removed_taps` so existing machines untap the stale path and avoid a duplicate-formula conflict
