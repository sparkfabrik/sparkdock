...
- Added provisioner support to the AI coding harness sync: executable `*.sh` scripts shipped under the upstream cache `provisioners/` directory run automatically after the file-copy phase (`bin/sparkdock-agents-sync` invokes them with `sync`, `bin/sparkdock-agents-status` with `status`), letting tools ship CLI-based integrations that cannot be delivered as static files. Tool-agnostic — dropping a new provisioner needs no code change — and a failing provisioner is isolated so it never aborts the whole sync
