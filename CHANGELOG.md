...
- Fixed provisioning failing on machines that installed `skhd` before the rename: its Homebrew receipt still pinned `koekeishiya/formulae`, so the tap-qualified install assert failed; added a migration step that reinstalls `skhd` from `asmvik/formulae` to rebind the receipt (no-op once rebound or on fresh installs)
