...
- Added engram to the update-notification system: `sparkdock-check-updates engram` delegates to the CLI's own `engram update --check` (exit 3 when the CLI is not installed), and `sjust sf-engram-info` reports the local install state and whether an update is pending
...
- Added Codex CLI to the AI coding harness skills registry: `sf-harness-sync` creates `~/.codex/skills` and symlinks managed skills into it, and `sf-harness-status` reports a `CODEX` column
...
- Fixed the menu bar app build failing on machines with Command Line Tools only: it retries with Swift's native build system, a failed build reports the compiler error instead of a Command Line Tools hint, and `sjust sparkdock-menubar-reinstall` installs exactly one Command Line Tools package, the newest offered, instead of letting Software Update layer several
...
- Fixed provisioning aborting at the cask upgrade step with `Error: No casks found for google-cloud-sdk`: Homebrew renamed the Google Cloud SDK cask to `gcloud-cli`, so the package list no longer resolved and the Ansible conditions gating the gcloud component setup silently stopped matching. The package list, those conditions, the `system-gcloud-reconfigure` recipe and the `brew info` hint printed by the interactive installer now use `gcloud-cli`; the `share/google-cloud-sdk` paths are unchanged because the renamed cask still installs its artifacts there
...
- Fixed pkg-based cask installs (`docker-desktop`, `zoom`, ...) failing on every fresh interactive provisioning with `sudo: a terminal is required to read the password`: `--ask-become-pass` never defines the `ansible_become_pass` template variable, so the `homebrew_cask` `sudo_password` wiring and the fail-fast password validation silently never received the password. The playbook now collects it via a play-level `vars_prompt` (same `BECOME password:` prompt, also feeds privileged tasks), all entry points were aligned (justfile, sjust recipes, `sparkdock tui`, headless/tag-only runs suppress the prompt with `-e ansible_become_pass=`), `is_ci_environment()` treats a real terminal as interactive so stray CI-style variables can no longer silence the prompt, and the entrypoint prints which mode it picked
