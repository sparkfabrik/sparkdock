...
- Added `sjust sf-openspec-install-global [tools]` recipe and Ansible task that install or update OpenSpec skills and prompt commands globally in `~/.claude` (skills to `~/.claude/skills/openspec-*`, prompts to `~/.claude/commands/opsx/*`) via a home `~/openspec` project; idempotent (`openspec update --force` when already initialized, `openspec init --force` on first run), defaults to the `claude` tool, runs during provisioning when the `openspec` CLI is available
