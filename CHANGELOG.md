...
- Added `sjust macos-defaults-info` recipe that displays the curated macOS-defaults profile grouped by category, with per-setting descriptions and override examples; renders through `gum format` + `gum pager` when interactive, falls back to plain Markdown for piping
...
- Refactored `sjust macos-defaults*` recipes to thin wrappers; complex logic moved into standalone shell scripts under `sjust/scripts/macos-defaults/` (`lib.sh`, `apply.sh`, `undo.sh`, `docs.sh`, `info.sh`) to match the project's pattern for `openspec/configure.sh` and `rtk/setup.sh`
- Trimmed the curated macOS-defaults set to focus on security and correctness; strong personal preferences (dock auto-hide, smart-quotes substitution, tap-to-click, accent-picker disable, screencapture shadow, dock tile size / minimize effect, forced Finder list view, save-to-iCloud-by-default) are deliberately excluded from the curated profile and must be set via the user overrides file
