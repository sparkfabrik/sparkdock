...
- Added `/usr/local/bin/sparkfabrik-claude-code-otel-headers` — a small generic helper installed via the existing "Copy remaining system scripts" Ansible task. Claude Code calls it through the `otelHeadersHelper` field in the org managed-settings (private) to produce the OTLP `Authorization: Bearer …` header at runtime. The script reads the bearer from Secret Manager using the user's own gcloud session (no token at rest on disk, no MDM-pushed token fetcher); the Secret Manager secret/project come from env vars set by the private managed-settings file, so this public script stays generic
