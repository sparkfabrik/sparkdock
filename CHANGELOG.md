...
- Added globally managed optional AI harness skill categories with styled `sf-harness-category [list]|enable|disable` output, persistent user selection, conflict-safe removal, category health in `sf-harness-status`, and shared `sjust`/Linux `ajust` support; system skills remain mandatory and existing manifests migrate without reinstalling optional categories
