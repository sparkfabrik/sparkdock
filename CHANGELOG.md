...
- Added `sjust macos-doctor`, a pluggable macOS health-check command with eight checks: orphaned launchd jobs, WindowServer CPU (measured as a real time delta, not `ps %CPU`), local APFS snapshots, developer caches, Homebrew health, Spotlight index scope, reclaimable Docker disk, and shell startup time. The report path is read-only; `sjust macos-doctor-fix <check>` fixes one check at a time behind a confirmation, moving affected paths into a quarantine snapshot that `sjust macos-doctor-undo` restores, and asking twice for the checks whose fix cannot be reversed. Every fix can be previewed with `sjust macos-doctor-fix <check> dry-run`, which walks the same code path and changes nothing. `sjust macos-doctor-info <check>` explains a single check, which is where the deeper diagnostic recipes live. Adding a check means dropping a file into `sjust/scripts/macos-doctor/checks/`, with no catalog to edit. Wired into `sparkdock tui` as a `macOS doctor` status row (fed by a cheap-checks-only `count` mode) and a `Run diagnostics` action on `D`
