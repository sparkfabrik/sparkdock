#!/usr/bin/env zsh
# SSH Agent Plugin for Sparkdock
# Based on oh-my-zsh ssh-agent plugin
# Manages ssh-agent lifecycle and identity loading
...
function _start_agent() {
  ...
  if [[ ! -d "$HOME/.ssh" ]]; then
    echo "[sparkdock] ssh-agent plugin requires ~/.ssh directory"
    return 1
  fi
