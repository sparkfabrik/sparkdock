...
sparkdock_print_shell_overview() {
    ...
    echo "   ~/.config/spark/shell.zsh      — personal aliases and exports (single file)"
    echo "   ~/.config/spark/shell.d/*.zsh  — drop-in snippets for MDM/tool-managed config"
    echo "                                    (each owner manages its own file; sourced after shell.zsh)"
    echo ""
...
sparkdock_setup_ghostty_config() {
    ...
    if [[ -f "${USER_CONFIG}" && -s "${USER_CONFIG}" ]]; then
        ...
        if [[ -L "${USER_CONFIG}" ]]; then
            ...
            if grep -qE "config-file = \??user" "${USER_CONFIG}" 2>/dev/null; then
                ...
                echo ""
                echo "ℹ️  To use Sparkdock's managed Ghostty configuration, follow these steps:"
                echo ""
                echo "   1. Move your current customizations to the overrides file:"
                echo "      cp ${USER_CONFIG} ${USER_OVERRIDES}"
                echo ""
                echo "   2. Replace the contents of ${USER_CONFIG} with:"
                echo "      config-file = ${SPARKDOCK_CONFIG}"
                echo "      config-file = ?user"
                echo ""
                echo "   This loads Sparkdock defaults first, then your overrides from the 'user' file."
                echo "   The '?' prefix means the overrides file is optional (no error if missing)."
                echo "   See: https://ghostty.org/docs/config#splitting-into-multiple-files"
                return 0
            fi
        fi
    fi
...
clt_pick_softwareupdate_label() {
    ...
    labels="$(printf '%s\n' "${update_list}" \
        ...
        | grep 'Command Line Tools for Xcode' || true)"
    if [[ -z "${labels}" ]]; then
        return 1
    fi

    # Each label carries its version, e.g. "Command Line Tools for Xcode 26.6-26.6".
    versioned="$(printf '%s\n' "${labels}" | awk '
