...
The first supported call, read or write, is denied once while the guard requests any missing available required skills. For a publishing call the request also asks the agent to read any task-specific references required by those skills and revise its prepared text before retrying. The guard confirms only the parent skill load; the parent skill decides which reference applies, and reference files are not tracked separately. This first request is the only case in which the guard denies a call; if the load is not confirmed, the next attempt proceeds with a notice, as described below.

If the skill was already loaded, the first publishing call of each user turn is not blocked and carries a short review reminder as `additionalContext`. The reminder tells the agent to apply the parent skill and any task-specific reference it requires. The hook returns no permission decision, so the normal approval flow still applies to the call. The reminder reaches the model together with the tool result. It states that the text was not evaluated. Later writes in the same user turn proceed without another reminder. A new user prompt resets only the reminder; it does not force a full reload.
